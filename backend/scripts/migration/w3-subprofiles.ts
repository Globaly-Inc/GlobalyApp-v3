...
/**
 * The columns of `table` that Postgres actually stores as a date, restricted to
 * the ones this transform writes. Asking the catalogue rather than the column
 * name is the difference between checking a date and checking a string that
 * happens to be called one.
 */
async function dateColumns(ctx: TransformContext, table: string, candidates: readonly string[]): Promise<string[]> {
...
export async function transformSubProfiles(ctx: TransformContext, allowedCodes: ReadonlySet<string>): Promise<void> {
  ...
  for (const spec of TABLES) {
    ...
    // sides of every REAL date column back and compare them through dateOnly().
    //
    // Only genuine `date` columns — V1's start_date/end_date are free text
    // ("2019", "MM-YYYY") on both sides and dateOnly() would return null for
    // both, which is not a comparison, it is a coincidence that looks like one.
    // Those are covered by Gate 2's content check instead.
    for (const column of await dateColumns(ctx, spec.target, shared)) {
