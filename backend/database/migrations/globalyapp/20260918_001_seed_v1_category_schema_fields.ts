...
// ever had the 3 generic course fields (20260916_004) applied uniformly to every category, so a
// category like "Insurance" had no fields of its own. Matched by category NAME since these rows
// are user-created via the admin UI in this repo (no seeded slugs to rely on), and onConflict on
// (entity_id, entity_type, key) makes this idempotent / safe to run more than once.
...
export async function down(): Promise<void> {
  // No-op — same reasoning as 20260916_004: onConflict().ignore() means we can't tell which rows
  // this migration actually created vs. already existed, so a blanket delete would risk destroying
  // unrelated fields (and, via cascade, any service values already saved against them).
}
