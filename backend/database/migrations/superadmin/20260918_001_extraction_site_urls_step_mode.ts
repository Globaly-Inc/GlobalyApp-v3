...
// writes it, the snapshot step reads it, the classifier writes `role` onto it, and queue_pages
// reads `role = 'course'` out of it — so every step is re-runnable from the table rather than
// from a fresh crawl, and an admin can exclude or re-role a URL BEFORE it is scraped or sent to
...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.withSchema(S).createTable("extraction_site_urls", (t) => {
    ...
    t.text("role").nullable(); // NULL until url_classify; then course | other
    t.text("role_source").nullable(); // heuristic | llm | admin
  ...
  await knex.raw(`CREATE INDEX idx_extraction_site_urls_job_role ON ${S}.extraction_site_urls (job_id, role) WHERE NOT excluded`);
