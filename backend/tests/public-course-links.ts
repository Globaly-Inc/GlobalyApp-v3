...
async function seed() {
  ...
  // Public visibility runs through the promoted institution: the course reads inner-join
  // `institutions` on the job, so a job without a published row is invisible by design.
  const [institution] = await masterKnex("institutions")
    .insert({
      institution_name: `Test Institution ${Date.now()}`,
      subdomain: `test-course-links-${Date.now()}`,
      source_job_id: jobId,
      is_published: true,
    })
    .returning("id");
  ...
  return {
    ...
    unusedUnitId: otherUnit.id as string,
  };
}

async function main() {
  const { jobId, courseId, taughtAt, institutionId } = await seed();
  try {
