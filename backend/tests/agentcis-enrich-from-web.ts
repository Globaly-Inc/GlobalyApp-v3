 ...
 * job only, import must be "done", real website only), that a valid call dispatches to the
 * normal JOBS queue, that a failed dispatch rolls the atomic claim back to "done" instead of
 * stranding it at "processing", and that two concurrent calls on the same job can't both claim
 * it (only one publishes). queueService.publish is mocked so no real LavinMQ connection opens.
 *
...
async function main() {
  ...
  const [dispatchFailsJob] = await masterKnex(`${S}.extraction_jobs`)
    ...
    .returning("id");
  const [raceJob] = await masterKnex(`${S}.extraction_jobs`)
    .insert({ institution_url: "https://enrich-web-test-race.invalid", source_type: "agentcis", status: "done" })
    .returning("id");
  ...
  try {
    ...
    // Two concurrent requests on the SAME "done" job — only one may claim it and publish.
    ...
    const results = await Promise.allSettled([
      enrichFromWebsite(raceJob.id, ADMIN_ID),
      enrichFromWebsite(raceJob.id, ADMIN_ID),
    ]);
    const succeeded = results.filter((r) => r.status === "fulfilled");
    const rejected = results.filter((r) => r.status === "rejected");
    assert(succeeded.length === 1, "exactly one of two concurrent enrich calls on the same job succeeds");
    assert(rejected.length === 1, "the other is rejected, not silently accepted");
    assert(
      rejected[0]?.status === "rejected" && (rejected[0].reason as Error).constructor.name === "BadRequestError",
      "the losing call is rejected with BadRequestError, not a different/unexpected error",
    );
    assert(calls.length === 1, "only ONE publish happened for the two concurrent calls — no duplicate crawl dispatched");
  } finally {
    queueService.publish = originalPublish;
    await masterKnex(`${S}.extraction_jobs`)
      .whereIn("id", [notAgentcisJob.id, notDoneJob.id, syntheticUrlJob.id, validJob.id, dispatchFailsJob.id, raceJob.id])
      .delete();
