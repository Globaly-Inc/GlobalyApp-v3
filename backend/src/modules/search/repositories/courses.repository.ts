...
/**
 * The joins and visibility rule every public course read shares — the list, the detail page and
 * the institution profile's facets all start here, so none of them can quote a fee the others
 * don't or show a course the others hide.
 *
 * Inner join on institutions (at most one per job, institutions_source_job_uniq, so it cannot fan
