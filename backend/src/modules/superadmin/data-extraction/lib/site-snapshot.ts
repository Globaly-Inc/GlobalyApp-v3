...
import { getPage, normaliseUrl } from "./page-store.js";
...
/**
 * GCS object path for a page. Pure, and deterministic per URL, so a rerun overwrites that page's
 * own file rather than accumulating copies.
 *
...
export function snapshotPathFor(url: string): string {
  ...
  return `${SNAPSHOT_PREFIX}/${siteOf(url)}/${host}/${slug.slice(0, 180)}-${digest}.md`;
}
...
export async function snapshotSite(jobId: string, urls: string[], batch?: SnapshotBatch): Promise<number> {
  if (!isConfigured()) {
