...
export const INTEGRATION_KEYS = ["higgsfield_api_key", "gsc_service_account_json", "gsc_site_url", "globalyos_crm_api_key", "globalyos_crm_url", "pexels_api_key"] as const;
export type IntegrationKey = (typeof INTEGRATION_KEYS)[number];

export const UpdateIntegrationsSchema = z.object({
  higgsfield_api_key: z.string().trim().max(4096).optional(),
  pexels_api_key: z.string().trim().max(4096).optional(),
