...
// ─── Institution sub-resource ids ────────────────────────────────────────────
// Business sub-resources (fees, intakes, ...) live in a tenant table with integer `increments`
// ids. Institution sub-resources are extraction_* rows (uuid ids) instead — see
// institution-courses.repository.ts — so their route params need a uuid variant.
export const InstitutionSubIdParamSchema = z.object({ id: z.coerce.number().int().positive(), subId: z.string().uuid() });
export const InstitutionFeeIdParamSchema = InstitutionSubIdParamSchema.extend({ feeId: z.string().uuid() });
export const InstitutionIntakeIdParamSchema = InstitutionSubIdParamSchema.extend({ intakeId: z.string().uuid() });
export const InstitutionEligibilityIdParamSchema = InstitutionSubIdParamSchema.extend({ eligibilityId: z.string().uuid() });
export const InstitutionStudyOptionIdParamSchema = InstitutionSubIdParamSchema.extend({ optionId: z.string().uuid() });
export const InstitutionStudyUnitIdParamSchema = InstitutionSubIdParamSchema.extend({ unitId: z.string().uuid() });
export const InstitutionAccreditationRowIdParamSchema = InstitutionSubIdParamSchema.extend({ rowId: z.string().uuid() });

// Institution accreditations link to the same global public.accreditations catalog (integer ids)
// as the business twin's service_accreditations.accreditation_id — see the repository's comment.
export const InstitutionServiceAccreditationInputSchema = z.object({
  accreditation_id: z.number().int().positive(),
});
