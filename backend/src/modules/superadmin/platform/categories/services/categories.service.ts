...
// Standard "Course details" fields every service category gets on creation — see the
// 20260916_004_seed_course_schema_fields.ts migration's comment for why these must exist
// per-category rather than being assumed: the service form writes/reads values keyed by a
// real schema_fields id, and silently drops the selection when none exists for the category.
const STANDARD_COURSE_FIELDS: SchemaFieldInput[] = [
