...
export async function businessServicesRoutes(app: FastifyInstance) {
  ...
  // A service's own cover image. With none set the client falls back to the business's cover, so
  // DELETE here means "go back to the business cover", not "show nothing".
  app.post("/services/:subId/cover", { preHandler: requireBusinessContext }, async (req, reply) => {
    const { subId } = SubIdSchema.parse(req.params);
    // Before the upload, not after: an unknown UUID would otherwise leave an orphaned object.
    await service.requireService(Number(req.business!.id), subId);
    const file = await req.file();
    if (!file) throw new NotFoundError("No file uploaded");
    const buffer = await file.toBuffer();
    storage.validateFile(file.mimetype, buffer.length);
    const storagePath = storage.buildPath("public/businesses", req.auth.orgId!, `services/${subId}`, file.filename);
    await storage.uploadFile(storagePath, buffer, file.mimetype);
    const updated = await service.setServiceCover(Number(req.business!.id), subId, storagePath);
    await activityService.logActivity(req.db, Number(req.auth.sub), "SERVICE_UPDATED", "service", subId);
    return reply.send(updated);
  });

  app.delete("/services/:subId/cover", { preHandler: requireBusinessContext }, async (req, reply) => {
    const { subId } = SubIdSchema.parse(req.params);
    await service.setServiceCover(Number(req.business!.id), subId, null);
    await activityService.logActivity(req.db, Number(req.auth.sub), "SERVICE_UPDATED", "service", subId);
    return reply.status(204).send();
  });
