// Driver selection, shaped like shared/storage/storageService.ts: a real provider when configured,
// otherwise a local stand-in, and callers never learn which one is active.
...
export { makeDevSessionId, isDevSessionId } from "./dev-driver.js";
 ...
 * typecheck, an unrelated worker, a test suite. It would also fire in a production deployment that never
 * takes a payment. The condition that actually matters is narrower — "we are about to hand a caller a
 * driver that approves payments for free, in production" — and that is exactly what this checks.
 */
