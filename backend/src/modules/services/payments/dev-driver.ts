...
// locally or in tests. A deliberately mismatched id is how the amount and currency rejections are proven.
//
// Selected only when STRIPE_SECRET_KEY is unset. getDriver() refuses to hand it back in production; see
// index.ts for why the guard lives there and not here.
...
/** Build a dev session id. Used by the seed script and the tests — the only producers of these. */
...
// ─── In-memory Stripe stand-in ─────────────────────────────────────────────
// Process-local, which is all a dev/test driver needs. Refunds are keyed by PaymentIntent (what
// listRefunds queries) and separately by idempotency key (what a replay hits).
...
export const devDriver: PaymentDriver = {
  ...
  async retrieveSession(sessionId: string): Promise<PaymentSession> {
    // An unparseable id is reported as an unusable session rather than throwing, so a bad `?session_id=`
    ...
    const amountTotalMinor = Number.isInteger(Number(amount)) ? Number(amount) : null;
    return {
      sessionId,
      paymentStatus,
      amountTotalMinor,
      // Lowercased on purpose: Stripe returns lowercase currency, so the case-normalising comparison in
      // orders.service is exercised here too rather than only against real Stripe.
...
// ─── Test surface ──────────────────────────────────────────────────────────
// Only ever called from tests. It lives on the dev driver rather than behind an injection seam because the
// dev driver is already the non-production path — there is nothing here for production code to reach.
...
/** Pre-seed a refund Stripe "already holds", with no local row — the Stripe-succeeded/DB-failed state. */
