...
export const OPEN_ORDER_STATUSES: OrderStatus[] = ["pending_payment", "paid", "disputed"];
...
// price_minor, not price: money crosses the wire as an integer minor amount in both directions, so no
// amount is ever a float in JSON. The single major↔minor conversion lives in the form.
const priceMinor = z
...
export const CreateListingSchema = z
  ...
  // .strict() so provider_id, avg_rating, total_reviews, total_orders and any id in the body are rejected
  // loudly rather than silently stripped. The owner always comes from the JWT and every derived figure is
  // server-maintained.
  .strict()
  // A city belonging to another country would render as a mismatched location; the service verifies the
  // pair against the cities table, and this catches the obvious half-set case first.
  .refine((v) => !v.city_id || !!v.country_id, {
    message: "Pick a country before a city",
    path: ["city_id"],
  });

export const UpdateListingSchema = CreateListingSchema.innerType().partial().strict().refine(
