// Listing management for the seller: CRUD, pause/resume, cover upload, and the delete guard that V2 did
// not have.
...
// spreadsheets) and passed per-call, so nothing else in the app is widened by this feature.
...
/**
 * Mint the signed cover URL per read — they expire, so it cannot be stored. A failure to sign degrades that
 * one card to no image rather than failing the whole list.
 */
...
export async function create(providerId: number, input: CreateListingInput): Promise<ListingDto> {
  ...
  const hydrated = await repo.findListingById(row.id);
...
export async function update(id: number, userId: number, input: UpdateListingInput): Promise<ListingDto> {
  ...
  // A patch may move either half of the pair, so validate the resulting combination, not the input alone.
  const countryId = input.country_id !== undefined ? input.country_id : existing.country_id;
  const cityId = input.city_id !== undefined ? input.city_id : existing.city_id;
  ...
  // Changing the country without naming a city clears the stale city rather than leaving a mismatch.
  const clearedCity =
    input.country_id !== undefined && input.country_id !== existing.country_id && input.city_id === undefined
      ? { city_id: null }
      : {};

  await repo.updateListing(id, { ...input, ...clearedCity });
  const hydrated = await repo.findListingById(id);
  return toDto(hydrated!);
}
