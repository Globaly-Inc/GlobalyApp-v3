...
// ponytail: no verification_status filter. Every extracted course is
// 'unverified' today, so gating on it would return an empty list. The column is
// returned so a product decision to hide unverified courses can be made later.
...
export async function countCourses(): Promise<number> {
  const [{ count }] = await masterKnex(T).count("id as count");
  return Number(count);
}
