...
export const REFERRAL_CONFIG = {
  ...
  w1_days: 30,
  /** W2 — activation (signed_up_at) to qualification. Enforced when the referral qualifies. */
  w2_days: 90,
} as const;
...
/**
 * What each action type is worth.
 *
 * Still the single source of truth for the amounts the program ADVERTISES, and what the eventual payout
 * step will read. Nothing in this module debits or credits anything today — credits are a separate
 * feature — so this is a published price, not a posted transaction.
 */
...
/** The two owner kinds a referral code or a referrer can be. Declared here rather than imported from
 *  the credits module, so referrals has no dependency on a feature being rebuilt elsewhere. */
export type ReferralOwnerType = "user" | "business";

/**
 * Referral lifecycle states.
 *
 * This phase writes only `signed_up` and `qualified`. `credited` remains for rows credited before
 * credits became a separate feature, and is where the payout step will land once it is linked back in;
 * expired/voided/rejected are reserved for Phase 2/3. No code here writes any of those.
 */
