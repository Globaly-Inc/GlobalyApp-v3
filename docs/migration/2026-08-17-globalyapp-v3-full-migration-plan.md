...
**Date:** 2026-08-17 · **Re-verified against `staging` @ `64b3165` (2026-08-17, post-merge of PRs #49–#53)** · **Status:** awaiting approval — no implementation starts until this plan is signed off.
...
- `staging-mvp` is cut once from `staging` (@ `64b3165` or later) and is the base for every wave.
...
- In-flight branches that overlap program waves (`dev-feat-ai-knowledge`, `dev-feat-ai-counsellor-p1/-p2`, `dev-feat-personal-my-services` booking flow) must land into `staging` or `staging-mvp` **before** their wave dispatches (E1/E2/G3), so waves build on them, not beside them.
...
**Current state** (re-verified on `staging` @ `64b3165`). V3 has a solid vertical slice: OTP/JWT auth, platform users + sub-profiles, businesses with tenant-schema provisioning, agents/roles/permissions, social feed (no comments), blog (public module + admin CMS), the full extraction console, and — new since the PDF — public geo/search/compare (`geo`, `search` modules + `(web)` pages), the student-services marketplace (`other-services` module, merged), business branches/representations/partners, a thin tenant `business_services` table, `student_jobs`, and soft-delete across modules. AI-knowledge and AI-counsellor are in flight on unmerged branches. The frontend still has **more pages than the backend has endpoints** — admin monitoring/revenue/marketing pages and parts of the personal portal render against `mock-data.ts` files. There are zero tests and no CI gates. Migration tooling in-repo is one script ([backend/scripts/import-v2.ts](../../backend/scripts/import-v2.ts)).
...
**Major decisions preserved from the PDF:** V3 architecture is inviolable; 100% parity (Wave G); institutions become claimable (nullable owner); academic tests via discriminator column; canonical reference tables in `public`; drop `scrape_smoke_results` + `extraction_job_events`; messaging rebuilt not migrated.
...
### 3.4 Extraction (V3's strong suit — DONE)

Jobs/queue/staging/review/junctions, visas + MARA, AgentCIS import, aggregators, accommodation/insurance/banking/visa-services/test-prep/career/translation/transport verticals, AI memory: all **DONE** under [superadmin/data-extraction](../../backend/src/modules/superadmin/data-extraction) with schemas in [migrations/superadmin/](../../backend/database/migrations/superadmin/). Data: **W4 = check-then-delta** — `import:v2` rehearsals may already have loaded the corpus (annex decision #5).
...
| AI counsellor | `ai-counselor`, `business-ai-counselor`, `migrate-guest-chat`; 301 messages | `apps/ai-service` (ReAct, metering, fail-closed) | **MISSING** (`personal/ai` page mock) | [R][V2][M] | E2 / W7 |
| Knowledge base + RAG | `ai-knowledge-crawl-source/-discover/-embed/-suggest-rows`; 207 docs, 58 sources | V2 RAG (`knowledge_chunks`, hybrid retrieval, recall@5 gate) | **IN FLIGHT** — `dev-feat-ai-knowledge` branch (module + `ai_knowledge` migration), not on `staging`; land before E1 | [R][V2][M] | E1 / W7 |
| AI counsellor implementation branches | — | — | **IN FLIGHT** — `dev-feat-ai-counsellor-p1`/`-p2`, not on `staging`; land before E2 | — | E2 |
...
| Visas/MARA public directory | — | `visas.ts` | Backend data DONE (extraction); public API/pages MISSING | G1 |
...
| AI-embed widget | `ai-embed-content`, `ai-embed-validate` | — | MISSING | G4 |
...
| `scrape_smoke_results`, `extraction_job_events` | — | — | drop with reason |
...
| E | `superadmin/`+`business/` | knowledge_chunks (pgvector — extension already used by [20260811_001_extraction_memory_embedding.ts](../../backend/database/migrations/superadmin/20260811_001_extraction_memory_embedding.ts)), ai_usage_events, counsellor sessions/messages, scribe_* (incl. consent log), lms_*/training_* |
| G | per feature | from V2 `apps/core-api/src/db/schema/schema.ts`, transformed to V3 conventions |
...
| E1–E4 | extend [superadmin/ai-knowledge](../../backend/src/modules/superadmin/ai-knowledge) (this branch); `counsellor/`, `scribe/`, `lms/` modules *(NEW)* | V2 `apps/ai-service` designs; V1 scribe fns as behavioral spec |
...
All PDF §6 + annex §8 risks carried forward (silent country FKs → W1; 19 supabase URLs → W6; snapshot drift → cutover re-extract + mapping diff; phantom endpoints → A4+D6 contract test; iCloud workspace hazard; `github-policies` rulesets referencing the wrong repo). From re-inspection of `staging` @ `64b3165`:
- **Unmerged in-flight branches** — `dev-feat-ai-knowledge` (E1 base), `dev-feat-ai-counsellor-p1/-p2` (E2 base), `dev-feat-personal-my-services` booking flow (G3): each must land into `staging-mvp` before its wave dispatches, or the wave builds a competing implementation.
...
| 4 | Drop smoke-results + job-events | **Drop both** |
