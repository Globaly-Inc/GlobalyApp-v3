import type { ComposeWithAiInput, CreatePostInput, FeedPage, FeedPost, ListFeedParams, PostMedia, ReactionGroup } from "./types";
...
export const feedMockApi = {
  // Mock mode has no server to enforce context, so `businessId` only reaches the log here — the visibility
  // matrix is exercised against the real API.
