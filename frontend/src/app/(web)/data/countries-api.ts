...
type FeaturedCountryRow = { id: number; name: string; slug: string | null; flag_emoji: string | null };
...
export async function getFeaturedCountries(): Promise<Destination[]> {
  ...
  return countries.map((c) => ({ id: String(c.id), name: c.name, slug: c.slug ?? "", flagEmoji: c.flag_emoji ?? "🌐" }));
}
