...
export function PartnershipConnectMockup() {
  ...
  useEffect(() => {
    ...
    const toNext = setTimeout(() => setIndex((i) => (i + 1) % AGENCIES.length), CYCLE_MS);
    return () => {
      clearTimeout(toHold);
      clearTimeout(toOut);
      clearTimeout(toNext);
  ...
  // index is always in range (initialized to 0, only ever advanced via `% AGENCIES.length`)
  const agency = AGENCIES[index]!;
  const AgencyIcon = agency.Icon;

  const cardTransform = phase === "in" ? "translateY(40px)" : phase === "out" ? "translateY(-40px)" : "translateY(0)";
      ...
      <div className="relative py-4">
        <div className="grid grid-cols-2 gap-3 relative z-10 items-stretch">
          ...
          <Card className="p-4">
            ...
            <Badge variant="secondary" className="mt-2 text-[10px]">Institution · Verified</Badge>
          </Card>
        </div>

        {/* Connector line */}
        <svg className="absolute left-0 right-0 top-1/2 -translate-y-1/2 z-0 pointer-events-none" width="100%" height="2" aria-hidden="true">
          <line
            x1="15%"
            y1="1"
            x2="85%"
