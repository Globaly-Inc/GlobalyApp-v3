 ...
 * capability chips.
 *
 * V2's six action cards (start a DM, create a space, send a group message, post an
 * announcement, share a file, search messages) are deliberately NOT reproduced: five of
 * them create things the student side does not have, and search already lives in the
 * sidebar. What replaces them is the one sentence that actually answers "why is this
 * empty" — a conversation appears when a business unlocks an enquiry.
 */
export function ChatEmptyState({ threadCount }: Readonly<{ threadCount: number }>) {
  ...
  const profileFirstName = useAppSelector((s) => s.profile.profile?.first_name);
  // Withheld until hydrated so the first client render matches the server's HTML.
  const firstName = hydrated ? profileFirstName : undefined;
    ...
    <div className="flex h-full flex-col items-center justify-center px-4 py-8">
      <div className="relative mb-6">
        <div className="flex size-20 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/20 via-primary/10 to-primary/5 shadow-lg">
          <Image src={ICON.src} alt="Globalyapp" width={ICON.width} height={ICON.height} className="size-12 rounded-lg" />
        </div>
      ...
      <h1 className="mb-2 text-center text-2xl font-bold">
        Hi{firstName ? ` ${firstName}` : ""}! Welcome to Messages
      </h1>
      <p className="text-center text-muted-foreground">Your conversations with agents and institutions</p>
      ...
      <p className="mt-6 max-w-md text-center text-xs text-muted-foreground">
        {threadCount > 0
          ...
          : "A conversation opens as soon as a business unlocks one of your enquiries — you'll find it here."}
      </p>
    </div>
  );
}
