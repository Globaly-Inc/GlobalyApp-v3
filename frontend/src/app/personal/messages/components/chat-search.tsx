...
export function ChatSearch({
  ...
  const results = useMemo(() => {
    ...
    const conversations = threads.filter(
      (t) => t.business_name.toLowerCase().includes(q) || t.course_name.toLowerCase().includes(q),
    );

    const messages: Array<{ message: EnquiryMessage; thread: MessageThreadSummary }> = [];
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground">
                      <Highlight text={thread.business_name} query={query.trim()} />
                    </p>
