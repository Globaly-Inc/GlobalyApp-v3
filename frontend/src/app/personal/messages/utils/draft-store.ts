...
export function saveDraft(distributionId: string, businessName: string, content: string) {
  const drafts = { ...read() };
  if (!content.trim()) {
    if (!drafts[distributionId]) return; // nothing to change, so don't wake subscribers
    delete drafts[distributionId];
  } else {
    drafts[distributionId] = { distributionId, businessName, content, updatedAt: new Date().toISOString() };
  }
  write(drafts);
}
