...
export const activityDate = (t: MessageThreadSummary) => new Date(t.last_message_at ?? t.unlocked_at);
...
export function conversationToText(
  thread: Pick<MessageThreadSummary, "business_name" | "course_name">,
  messages: EnquiryMessage[],
): string {
  const lines = [`Conversation with ${thread.business_name}`, `Course: ${thread.course_name}`, ""];
  let lastDay = "";
