...
export function RecordSections({
  ...
  // Preview is what a visitor sees: a section the owner marked private must not render at all,
  // not just lose its edit controls — showing the records with only the buttons stripped would
  // misrepresent what the "public" profile actually exposes.
  const showEducation = !readOnly || isSectionPublic("education");
  const showWorkExperience = !readOnly || isSectionPublic("work_experience");
  const showAcademicTests = !readOnly || isSectionPublic("academic_tests");
  const showLanguageTests = !readOnly || isSectionPublic("language_tests");
