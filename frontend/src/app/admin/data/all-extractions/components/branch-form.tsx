...
export function BranchForm({
  ...
  // Tracks whether map_link was derived from the address fields (vs. entered by hand),
  // so a later address edit can refresh it instead of leaving a stale generated link.
  const [mapLinkAutoGenerated, setMapLinkAutoGenerated] = useState(false);

  const set = <K extends keyof BranchValues>(key: K, value: BranchValues[K]) => {
    setValues((v) => ({ ...v, [key]: value }));
    if (key === "map_link") setMapLinkAutoGenerated(false);
  ...
  // Once a map link has been auto-generated, keep it in sync with the address fields it was
  // derived from — otherwise a later correction to city/state/postcode/country silently leaves
  // the branch pointing at its previous location.
  useEffect(() => {
    if (!mapLinkAutoGenerated || !values.address.trim()) return;
  ...
  }, [values.city, values.state, values.postcode, values.country]);
      ...
      <CardContent className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Textarea
            ...
            onBlur={() => {
              if ((values.map_link.trim() && !mapLinkAutoGenerated) || !values.address.trim()) return;
              const query = [values.address, values.city, values.state, values.postcode, values.country]
                .filter(Boolean)
                .join(", ");
              setValues((v) => ({
