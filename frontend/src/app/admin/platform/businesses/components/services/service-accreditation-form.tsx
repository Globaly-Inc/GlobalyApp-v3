...
// Same-to-same reuse of the data-extraction "Add Accreditation" form (accreditation-library-section.tsx's
// LibraryForm) — same banner copy, same Name/Issuing Org/Website/Description fields. Adapted to this
// schema's normalized issuing_organization_id (FK) in place of the extraction library's free-text
// issuing_organization column: typing an org name here finds-or-creates it, then resolves to an id.
export function ServiceAccreditationForm({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (issuingOrg.trim()) {
        const existing = await categoriesApi.getIssuingOrganizations({ search: issuingOrg.trim(), limit: 1 });
        const match = existing.data.find((o) => o.name.toLowerCase() === issuingOrg.trim().toLowerCase());
        issuingOrgId = match ? match.id : (await categoriesApi.createIssuingOrganization(issuingOrg.trim())).id;
      }
      const created = await categoriesApi.createAccreditation({
        name: name.trim(),
