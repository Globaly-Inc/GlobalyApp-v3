...
function useServiceSummaryData(kind: Kind, orgId: number, serviceId: string) {
...
export function ServiceSummaryBodyExtras({
  ...
  const { eligibility, studyUnits, accreditations, loading } = useServiceSummaryData(kind, orgId, serviceId);
...
export function ServiceSummarySidebarExtras({
  kind, orgId, serviceId, name, hasCategory, description, isCourse, onNavigateTab,
}: Readonly<{
  ...
  const { fees, intakes, eligibility, loading } = useServiceSummaryData(kind, orgId, serviceId);
  if (loading) return null;
