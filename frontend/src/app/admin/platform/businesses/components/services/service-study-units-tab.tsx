...
export function ServiceStudyUnitsTab({
  kind,
  orgId,
  serviceId,
  ...
  useEffect(() => {
    ...
    const load = kind === "business" ? businessesApi.getServiceStudyUnits : businessesApi.getInstitutionServiceStudyUnits;
  ...
  const handleSave = async (input: ServiceStudyUnitInput) => {
    ...
    try {
      if (editing) {
        const update = kind === "business" ? businessesApi.updateServiceStudyUnit : businessesApi.updateInstitutionServiceStudyUnit;
        ...
        const create = kind === "business" ? businessesApi.createServiceStudyUnit : businessesApi.createInstitutionServiceStudyUnit;
      ...
      toast.error("Couldn't save study unit", { description: (e as ApiError).message });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    const del = kind === "business" ? businessesApi.deleteServiceStudyUnit : businessesApi.deleteInstitutionServiceStudyUnit;
    try {
      await del(orgId, serviceId, id);
      setRows((r) => r.filter((x) => x.id !== id));
      toast.success("Study unit removed");
    } catch (e) {
      toast.error("Couldn't remove study unit", { description: (e as ApiError).message });
    }
  };
