...
import { UNIT_TYPE_OPTIONS } from "@/app/admin/data/all-extractions/const";
import type { ServiceStudyUnit, ServiceStudyUnitInput } from "../../apis/types";

// Same-to-same reuse of the data-extraction "Add Study Unit" form (study-units-tab.tsx's StudyUnitForm).
const studyUnitSchema = z.object({
  name: z.string().trim().min(1, "Unit name is required"),
  code: z.string().trim(),
  points: z.string().trim().refine((v) => !v || !Number.isNaN(Number(v)), { message: "Credit points must be a valid number" }),
});

export function ServiceStudyUnitForm({
  unit,
  saving,
  onCancel,
  onSave,
}: Readonly<{
  unit?: ServiceStudyUnit;
  saving: boolean;
  onCancel: () => void;
  onSave: (values: ServiceStudyUnitInput) => void;
}>) {
  ...
  const [type, setType] = useState<ServiceStudyUnitInput["unit_type"]>(unit?.unit_type ?? "compulsory");
  const [description, setDescription] = useState(unit?.description ?? "");
  const [errors, setErrors] = useState<Record<string, string>>({});

  const submit = () => {
    const result = studyUnitSchema.safeParse({ name, code, points });
    if (!result.success) {
    ...
    <Card className="border-primary/40">
      ...
      <CardContent className="flex flex-col gap-4">
            ...
            {UNIT_TYPE_OPTIONS.map((option) => (
              <label key={option.value} className="flex cursor-pointer items-center gap-2 text-sm">
                <input
                  ...
                  onChange={() => setType(option.value as ServiceStudyUnitInput["unit_type"])}
                />
                {option.label}
              </label>
            ))}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Textarea id="unit-description" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Brief description…" />
        </div>
