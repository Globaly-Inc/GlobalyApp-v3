...
const STUDY_MODE_LABELS: Record<string, string> = { on_campus: "On campus", online: "Online", blended: "Blended" };
const STUDY_LOAD_LABELS: Record<string, string> = { full_time: "Full time", part_time: "Part time" };

export function ServiceStudyOptionsTab({
  kind,
  orgId,
  serviceId,
  ...
  useEffect(() => {
    ...
    const load = kind === "business" ? businessesApi.getServiceStudyOptions : businessesApi.getInstitutionServiceStudyOptions;
  ...
  const handleSave = async (input: ServiceStudyOptionInput) => {
    ...
    try {
      if (editing) {
        const update = kind === "business" ? businessesApi.updateServiceStudyOption : businessesApi.updateInstitutionServiceStudyOption;
        ...
        const create = kind === "business" ? businessesApi.createServiceStudyOption : businessesApi.createInstitutionServiceStudyOption;
      ...
      toast.error("Couldn't save study option", { description: (e as ApiError).message });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    const del = kind === "business" ? businessesApi.deleteServiceStudyOption : businessesApi.deleteInstitutionServiceStudyOption;
    try {
      await del(orgId, serviceId, id);
      setRows((r) => r.filter((x) => x.id !== id));
      toast.success("Study option removed");
    } catch (e) {
      toast.error("Couldn't remove study option", { description: (e as ApiError).message });
    }
  };
