...
import type { InstitutionCourse, InstitutionDetail } from "../../apis/types";
...
// Admin twin of business/profile's ViewInstitutionDrawer — admin already has unrestricted
// access to institution detail/courses, so this hits the plain admin endpoints directly.
export function ViewInstitutionDrawer({
  ...
  useEffect(() => {
    ...
    businessesApi.getInstitutionDetail(institutionId).then(setDetail).finally(() => setDetailLoading(false));
  }, [open, institutionId]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      businessesApi
        .getInstitutionCourses(institutionId, { search: search || undefined, page: 1, limit: PAGE_SIZE })
  ...
  const handlePageChange = (p: number) => {
    ...
    setCoursesLoading(true);
    businessesApi
      .getInstitutionCourses(institutionId, { search: search || undefined, page: p, limit: PAGE_SIZE })
      .then((res) => { setCourses(res.data); setCoursesTotal(res.total); })
      .finally(() => setCoursesLoading(false));
  };
      ...
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-xl py-5">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            <Building2 className="h-4 w-4" /> {detail?.business_name ?? "Institution"}
          </SheetTitle>
          <SheetDescription>Institution detail and course catalogue.</SheetDescription>
        </SheetHeader>
          ...
          {!detailLoading && detail && (
            <div className="flex flex-col gap-2 rounded-lg border bg-muted/30 p-3 text-sm">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <Link href={`/institution/${detail.slug}`} target="_blank" className="text-foreground underline underline-offset-2">
