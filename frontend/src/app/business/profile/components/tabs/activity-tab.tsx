...
import { fetchActivity } from "../../store/business-profile-detail-slice";
...
export function ActivityTab({ businessId }: Readonly<{ businessId: number }>) {
  ...
  const { items: activity, status, total } = useAppSelector((state) => state.businessProfileDetail.activity);
