...
type FeeInstallmentRow = { label: string; lines: FeeInstallmentLine[] };

function toFormInstallments(fee?: ServiceFee): FeeFormInstallment[] {
  const installments = fee?.installments as unknown as FeeInstallmentRow[] | undefined;
  if (installments?.length) {
    return installments.map((i) => ({ label: i.label, lines: i.lines.map((l) => ({ fee_type: l.fee_type, amount: String(l.amount) })) }));
  }
