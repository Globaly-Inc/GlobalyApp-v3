...
import type { AcademicTest } from "@/app/admin/data/all-extractions/apis/types";
...
type LanguageTestRow = { test_type_name?: string; overall_score?: string; [key: string]: string | undefined };
...
export function ServiceEligibilityForm({
  ...
  const [languageTests, setLanguageTests] = useState<LanguageTestRow[]>((requirement?.language_tests as LanguageTestRow[] | undefined) ?? []);
  const [academicTests, setAcademicTests] = useState<AcademicTest[]>((requirement?.academic_tests as unknown as AcademicTest[] | undefined) ?? []);
  const [errors, setErrors] = useState<Record<string, string>>({});

  const scoreLabel = SCORE_TYPE_OPTIONS.find((o) => o.value === scoreType)?.label ?? "Score";

  const patchLanguage = (index: number, patch: Partial<LanguageTestRow>) =>
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      academic_tests: academicTests.filter((t) => t.test_name?.trim()) as unknown as Record<string, unknown>[],
    });
  };
