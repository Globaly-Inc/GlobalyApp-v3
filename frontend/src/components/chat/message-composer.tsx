...
import { applyFormat, type MarkFormat } from "./markdown";
import { deleteDraft, getDraft, saveDraft } from "./draft-store";
import {
  ACCEPT,
  AttachmentMenu,
  AttachmentPreviews,
...
export function MessageComposer({
  ...
  /**
   * Uploads one picked file and resolves its storage path. Injected because the two
   * sides post to different endpoints; everything else about the composer is identical.
   */
  onUploadAttachment: (file: File) => Promise<{ storage_path: string }>;
  placeholder?: string;
  /**
  ...
  const update = (next: string) => {
    setValue(next);
    if (persistDraft) saveDraft(distributionId, counterpartName, next);
  };
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    await Promise.all(
      entries.map(async (entry) => {
        try {
          const uploaded = await onUploadAttachment(entry.file);
          setPending((prev) =>
