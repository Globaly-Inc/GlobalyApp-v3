...
| `public/` | Logo, orb, favicon source, OG image, photography, motion clips |
...
- **`next.config.ts`** still allows remote images from `picsum.photos`, left
  over from placeholder photography that is no longer used. Safe to drop.
